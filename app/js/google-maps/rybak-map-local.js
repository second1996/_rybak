...
function setMarkers(map, locations) {
  ...
  $(document).on('click', '.stores-list-filters-results .store-card-trigger', function () {
    ...
    for (markerKey in markersArr) {
      if (markersArr[markerKey].__gm) {
        ...
        if (markersArr[markerKey].position.lat() == cardLat && markersArr[markerKey].position.lng() == cardLng) {
  ...
  markerCluster = new MarkerClusterer(map, cleanMarkersArr, {
    // apply clustering to map
    cssClass: 'rybak-gm-marker',
    // set custom class for every marker
    zoomOnClick: true,
    // zoom after click on marker
    averageCenter: true,
    // gridSize: 60,
